...
async function interact(user_choice) {
    try {
        ...
        await gateway.connect(ccp, { wallet, identity: 'userDis01', discovery: { enabled: false } });
        ...
        // Updating product information regarding amount, location, owner and price
            ...
            let choice = readline.question("Would you like to get the data from productUpdate1.json file? (y/n) ");

            if (choice == 'y') {
                try {
                    const jsonString = fs.readFileSync('./json_data/productUpdate1.json')
                    ...
                    await contract.submitTransaction('addProduct', product.name, new_product.amount, new_product.location, product.gtin, time, product.expiry, 'Dis1', new_product.price);
                ...
                await contract.submitTransaction('updateProduct', product.name, amount, loc, product.gtin, time, product.expiry, 'Dis1', price);
...
function main() {
    console.log("**** Logged in as Distributor 01 ****");
    console.log();
